...
def get_weights(particles, sensor_reading=None, sensor_stdev=SENSOR_STDEV):
    ...
    x_dist = scipi.stats.norm(x_read, sensor_stdev)
    y_dist = scipi.stats.norm(y_read, sensor_stdev)
    
    for particle in particles:
        x_prob = x_dist.pdf(particle[0])
        y_prob = y_dist.pdf(particle[1])
...
def resample_on_weights(particles, weights):
    ...
    for i in xrange(len(particles)):
        ...
        index = bisect_right(cutoffs, search_value)
        if index:
